...
# ─ Create rolling_mean_3 and rolling_mean_7
...
# ─ Prepare Prophet dataset: Select only relevant columns and rename Stock_Quantity to 'y'
...
# ┈ Prepare future dataframe for test set
...
print("📊 Model Evaluation Metrics After Adding Lag & Rolling Features:")
print(f"MAE: {mae:.2f}")
...
print("📊 Model Accuracy Metrics with Calendar Features:")
print(f"MAE: {mae:.2f}")
...
print("✅ Prophet model saved successfully as 'prophet_model_with_calendar.pkl'")
...
print(f"\n▶ Predicted stock quantity for {date_input.date()}: {predicted_stock:.2f}")
print(f"☀️ Season for {date_input.date()}: {get_season(date_input.month)}")
print(f"🎉 Retail season/festival: {get_retail_season(date_input, holiday_df)}")
